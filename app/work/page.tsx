...
import { mockArtworks, mockCollections, TAG_GROUPS } from '@/lib/utils/mockData'
...
export default function WorkPage() {
    ...
    }, [activeCollection, activeTags, sort])

    const activeCollection_obj = mockCollections.find(c => c.id === activeCollection)
    ...
    const availableTagsInScope = useMemo(() => {
        const scopedArtworks = activeCollection === 'all'
            ? mockArtworks
            : mockArtworks.filter(a => a.collectionId === activeCollection)
        ...
        <aside className="space-y-0">
            ...
            <div className="mb-6">
                ...
                <div className="space-y-0.5">
                    ...
                    >
                        <span>All Works</span>
                        <span className="text-neutral-400 text-xs tabular-nums">{mockArtworks.length}</span>
                    </button>
                    {mockCollections.map(col => {
                        const count = mockArtworks.filter(a => a.collectionId === col.id).length
                        return (
                            <button
                                key={col.id}
                        ...
                        <Reveal>
                            <div className="flex items-center gap-4 mb-6">
                                <span className="text-sm font-display italic text-accent-500" style={{ fontWeight: 500 }}>
                                    {String(mockCollections.indexOf(activeCollection_obj!) + 2).padStart(2, '0') || '01'}
                                </span>
